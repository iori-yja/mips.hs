...
exc (st , Rinst "srav" d s _ q)        = ctrl (\x y -> if ( testBit x 31 )
   						     then foldl setBit (x `shiftR` y) [(32-y)..31] else x `shiftR` y)
					 True Imm st d s q
exc (st , Rinst "srlv" d s _ q)        = ctrl shiftR True  Imm st d s q
exc (st , Iinst "addi" d s im)         = ctrl (+)    False Imm st d s im
exc (st , Iinst "subi" d s im)         = ctrl (-)    False Imm st d s im

exc (st@(Set (sr,rs,mem)), Iinst "sw" b r o) = Running $ Set (sr, rs, M.insert ((rs !! b) + o) (rs !! r) mem)
exc (st@(Set (sr,rs,mem)), Iinst "lw" b r o) = let p = memlook ( (rs !! b) + o ) mem
						   rg = ( take b rs ) ++ p `mod` (1 `shift` 32) : drop (b+1) rs
					       in  Running $ Set (sr, rg, mem)
					       where
